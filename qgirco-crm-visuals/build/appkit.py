# -*- coding: utf-8 -*-
"""
Realistic application-UI mockup kit for the QGIRCO requirement-realization cards.
...
def app_window(pf, x, y, w, h, breadcrumb, record_title):
    ...
    dom = "org.crm.dynamics.com" if pf["key"] == "microsoft" else "qgirco.lightning.force.com"
...
def _chrome_sf(pf, x, y, w, h, breadcrumb, record_title):
    ...
    for i, (ic, lb) in enumerate(utils):
        ...
        if i == 1:
            ux += 6
    s.append(txt(x + w - 16, uby + ub/2 + 4, "QGIRCO · Lightning Experience", size=8.8,
...
def build_card(pf, req, content_fn, idx_label):
    ...
    s.append(header(pf, "REQUIREMENT → REALIZED IN PRODUCT",
                    req["headline"],
                    f"How the {pf['name']} solution addresses this QGIRCO requirement"))
    railW = 372
    top = 124
    bot = H - FOOTER_H - 14
