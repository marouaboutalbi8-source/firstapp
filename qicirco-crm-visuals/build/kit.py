# -*- coding: utf-8 -*-
"""
Shared SVG layout engine + brand/icon library for the QICIRCO CRM solution visuals.
...
Company: QICIRCO — Qatar General Insurance & Reinsurance Company
"""
...
# ----------------------------------------------------------------------------
# QICIRCO customer brand chrome  (Qatar maroon + deep slate + gold)
# ----------------------------------------------------------------------------
QIC = dict(
    maroon="#8A1538",       # Qatar national maroon — customer primary
    maroon_dk="#5E0E26",
    slate="#16283A",        # deep slate/navy
    slate2="#22384E",
    gold="#C9A24B",
    ink="#1B2733",
    sub="#5A6B7B",
...
def qic_logo(cx, cy, r=26):
    """Abstract shield/pearl mark for QICIRCO on the maroon header."""
    s = []
...
def header(pf, section, title, subtitle):
    ...
    lx = MARGIN + 74
    s.append(txt(lx, 40, "QICIRCO", size=27, color="#ffffff", weight="800", spacing="2"))
...
def footer(pf, idx):
    ...
    s.append(txt(MARGIN, y + 28, "QICIRCO Enterprise CRM Implementation  ·  Technical Proposal",
